...
行列積問題は、2つの正方行列$A,B$が与えられたとき、その積$AB$を計算する問題である。
また、ここでは簡単のため有限体$\F_q$上の行列の演算を考える。
一般に$\F_q$上の演算は$O(\polylog q)$時間で行えるが、簡単のためここでは$O(1)$時間で行えると仮定する。
行列積の平均時計算量では、一様ランダムな行列が入力として与えられる場合を考える。
...
それを使って任意の入力に対して行列積を解くアルゴリズムを設計できることを示した。[^BLR93]
...
<summary style="display: list-item">証明</summary>
  ランダム行列の行列積を成功確率$0.9$で解くアルゴリズムを$M$とする。
  任意の入力$A,B\in\F_q^{n\times n}$に対して、$M$を使って次のように行列積を計算するアルゴリズム$M'$を考える：
  1. 一様ランダムな行列$R_1,S_1\sim\F_q^{n\times n}$を選び、$R_2 = A-R_1,S_2=B-S_1$とする。
