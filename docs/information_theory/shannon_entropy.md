...
シャノンエントロピーは情報理論における最も基本的なエントロピーの概念で、確率変数の不確実性や情報量を測る指標である。理論計算機科学においても、ランダムネス抽出器、プライバシー保護、通信理論など様々な場面で重要な役割を果たす。
...
> **定義 (シャノンエントロピー)**
>
> 有限集合 $\calX$ 上に値をとる確率変数 $X$ に対して、以下で定義される量 $\entropy(X)$ を $X$ の **シャノンエントロピー** という：
>
> $$
  \begin{align*}
...
シャノンエントロピーは確率変数 $X$ の不確実性を測る指標である。 $\entropy(X)$ が大きいほど、 $X$ の値が予測しにくいことを意味する。
...
> **定義 (条件付きエントロピー)**
>
> 有限集合$\calX,\calY$上に値をとる確率変数$X,Y$に対して、以下で定義される量$\entropy(X\mid Y)$を$X$の$Y$に関する**条件付きエントロピー**という：
>
> $$
  \begin{align*}
...
> **定義 (相互情報量)**
>
> 有限集合$\calX,\calY$上に値をとる確率変数$X,Y$に対して、以下で定義される量$I(X;Y)$を$X$と$Y$の**相互情報量**という：
>
> $$
  \begin{align*}
