...
**探索から判定への帰着**とは、判定問題を解くアルゴリズムを使って探索問題を解く方法全般を指す用語である。
埋め込みクリーク問題にもそのような帰着が存在し、与えられたグラフが埋め込みクリークかErdős--Rényiグラフかを判定するアルゴリズムを使って、
埋め込みクリークを見つけるアルゴリズムを設計することができる。[^AAKMRX07]

## 主張

少し問題設定が複雑なので、ここではinformalな主張と証明のアイデアを与える。
...
<summary style="display: list-item">帰着のアイデア</summary>
  ...
  $G=G(n,1/2)$ならば確率$1-1/n^2$で$\calA(G)=0$となる。

  さて、探索問題の入力を$G=\PC(n,k)$とし、その頂点集合$V=[n]$に対し、埋め込まれたクリークを$C\subseteq V$とする。
  各頂点$u\in V$に対し、$G$上の$u$とその隣接頂点からなる集合を$N(u)$とする。
  グラフ$G$から頂点集合$N(u)$を除去して得られるグラフ$G_u$を考えよう。

  {: align="center"}
  ![text]({{site.baseurl}}/docs/planted_clique/images/search_to_decision.svg)
  {: width=70%}

  - もし$u\in C$ならば、隣接頂点はクリークを含むので、$N(u)\supseteq C$となる。グラフ$G_u$は$G$に埋め込まれたクリークをもたないため、その周辺分布はおおまかにいうと$G(n/2,1/2)$となる。
  - もし$u \not\in C$ならば、クリーク$C$と$u$の間には平均的に$k/2$本の辺が存在するため、$\abs{N(u)\cap C}\approx C$となり、残った$G_u$にはサイズおよそ$k/2$のクリークを持つことになるため、周辺分布は大体$\PC(n/2,k/2)$となる。

  判定問題を解くアルゴリズムを使うとこれらのケースを確率$1-1/n^2$で区別できるため、$u\in C$かどうかを判定できる。
  これを全ての頂点$u\in V$に対して行うと、$u\in V$に関するunion boundにより、確率$1-1/n$で$C$を見つけることができる。
