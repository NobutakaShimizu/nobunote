...
擬似ランダム性とは端的にいうと「ランダムに見える」という性質を意味する。
この「ランダムに見える」という性質を定義するには、二つの分布の識別不可能性の概念を導入する。
...
> $$
  \begin{align*}
    {}^\forall f\in\calF,\quad \left| \Pr[f(X)=1] - \Pr[f(Y)=1] \right| \le \gamma
  \end{align*}
> $$
>
> を意味する。
...
{: .remark }
> - 擬似ランダム性は確率変数 $X,Y$ の分布にのみに依存する性質なので、$(X,Y)$ が独立かどうかは問わない。
> - 特に、関数族 $\calF$ が全ての $\binset$ 値関数からなる集合の場合、$\gamma$-識別不可能であることと $X$ と $Y$ の分布間の統計距離が $\gamma$ 以下であることは等価である（cf. [統計距離]({{site.baseurl}}/docs/information_theory/f_divergence/statistical_distance)）。

ある確率変数(または分布) $X$ が擬似ランダムであるとは、特定の関数族に対して一様分布と識別不可能であることを意味する。
...
が識別不可能であると定義する。ここで $X\sim\binset^n$ および $U\sim\binset$（$X$ と $U$ は独立）。
...
主に関数クラス $\calC$ としては多項式時間アルゴリズムや多項式サイズの回路を考える。
このように、制限された計算能力で計算できる関数族 $\calC$ に対する擬似ランダム性を **計算量的擬似ランダム性** という。

## 関数の平均時困難性と擬似ランダム性

平均時困難性と擬似ランダムネス性は密接に関連している。
そもそも、平均時の意味で困難な関数 $f\colon \binset^n\to\binset$ はどのような関数だろうか？
「計算が難しい」とは、入力 $x$ を見た時に $f(x)$ を推測するのが難しいということを意味する。
従って、$f(x)\in\binset$ が $x$ に依存せずランダムであれば推測することは不可能なので、難しそうな気がする。
これを決定的な関数 $f$ で模倣するためには $f(x)$ がランダムっぽく見える必要がある。
このように、$x\mapsto f(x)$ の計算困難性と $f(x)$ の擬似ランダム性には密接な関係がある。

具体的には、
関数 $f$ が強い平均時困難性を持つとき、一様ランダムな $x\sim \binset^n$ に対して $f(x)$ もまた擬似ランダムなビットになる。
これはYaoのnext-bit predictorという結果で形式的に証明されており、平均時計算量理論（特にハードコア補題の応用）を直感的に理解する上で非常に大切な視点になる。

ここでは $\calC=\SIZE(s)$ として、サイズ $s$ の回路で表現される関数族を考える。
...
で定める。このとき、$C'$ のサイズは $s+O(1)$ であり、$(x,b)\sim\binset^{n+1}$ のときは確率 $1/2$ で $1$ を出力する。
...
であるので、分布 $(x,f(x))$ はサイズ $s+O(1)$ に対して $\varepsilon$-擬似ランダムではない。

</details>

逆方向の主張（の対偶）はYaoのnext-bit predictorという結果を用いて証明される。
...
<summary style="display: list-item">証明</summary>
  ...
  が成り立つ（必要に応じて出力をフリップすることにより絶対値を外す）。
  ...
  が成り立つ。つまり、$C(x,b)$ は $b=f(x)$ のときにより $1$ を出力しやすい回路となっているわけであるので、
  $C(x,1)$ と $C(x,0)$ を実行してみて $C$ が $1$ を出力した方のビットを出力すれば、$f(x)$ を推測できそうな直感がある。
  ...
  と定義する。$C_0$ と $C_1$ の少なくとも一方が $C'$ として所望のものになっていることを主張する。
  ...
  より、$C_0$ と $C_1$ の少なくとも一方は $f$ を $(1/2+\varepsilon)$ の割合の入力で計算するので、確かに所望の回路 $C'$ の存在性は示せた。
</details>